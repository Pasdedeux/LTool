    ...
    public partial class GLoader3D : GObject, IAnimationGear, IColorGear
    {
        ...
        public string url
        {
            ...
            set
            {
                if (_url == value)
                {
                    if ((!m_IsLoadEnd) || _contentItem != null)
                    {
                        return;
                    }
                    else if (_content.wrapTarget)
                    {
                        _content.wrapTarget.SetActive(true);
                        LoadEnd?.Invoke();
                        LoadEnd = null;
                        return;
                    }
        ...
        public void SetWrapTarget(GameObject gameObject, bool cloneMaterial, int width, int height)
        {
            ...
            sourceWidth = Mathf.Max(1, width);
            sourceHeight = Mathf.Max(1, height);
        ...
        private bool m_IsLoadEnd = true;

        virtual protected void LoadExternal()
        {
            m_IsLoadEnd = false;
            System.Action<Object> end = (Object ap) =>
            {
                if (ap == null)
                {
                    m_IsLoadEnd = true;
                ...
                GameObject _content = GameObject.Instantiate<GameObject>(ap as GameObject);
                SetWrapTarget(_content, true, 0, 0);
                LoadEnd?.Invoke();
                LoadEnd = null;
                m_IsLoadEnd = true;
            };
            RsLoadManager.Instance.LoadAsync<GameObject>(url, end);
        ...
        public override float alpha
        {
            ...
            set
            {
                ...
                if (wrapTarget)
                {
                    Spine.Unity.SkeletonAnimation skeletonAnimation = wrapTarget.GetComponentInChildren<Spine.Unity.SkeletonAnimation>();
        ...
        public GoWrapper WrapperContent
        {
            get
            {
                return _content;
