...
public class FuncRecordManager : BaseLocalConfigManager<FuncRecordManager>
{
    ...
    {
        long oldDaySC;
        bool isHave = LocalData.FuncTime.TryGetValue(aName + "OpenFunc",out oldDaySC);
        if (!isHave)
        {
            return true;
        }
       
        long nowDay =LitTool.GetTimeStamp() / ConstantTool.TIME_DAY_SC;
    ...
    public long OpenFuncDisNowTimeS(string aName)
    {
        ...
        return LitTool.GetTimeStamp() - oldDaySC;
    }
    /// <summary>
    /// 获取功能刷新时间戳
    /// </summary>
    ...
    {
        aTime = aTime == -2 ? LitTool.GetTimeStamp() : aTime;
        LocalData.FuncTime[aName /*+ "OpenFunc"*/] = aTime;
        LocalData.SaveFlag();
    ...
    {
        LocalData.FuncNum[aName + "FuncNum"] = aNum;
        LocalData.SaveFlag();
    }
    /// <summary>
    /// 添加功能数量
    ...
    public void AddFuncNum(string aName, int aNum = 1)
    {
        ...
        int old= 0;
        bool isHave = LocalData.FuncNum.TryGetValue(aName + "FuncNum", out old);
        LocalData.FuncNum[aName + "FuncNum"] = aNum + old;
        LocalData.SaveFlag();
    }
    /// <summary>
    /// 获取功能数量
    ...
    {
        LocalData.FuncNum[aName + "FuncNum"] = 0;
        LocalData.SaveFlag();
    }
