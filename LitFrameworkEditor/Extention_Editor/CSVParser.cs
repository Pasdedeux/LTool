...
namespace LitFrameworkEditor.EditorExtended
{
    ...
    class CSVParser
    {
        ...
        string[] type;
        string ClassName;
        const string spaceName = "LitFramework";
        ...
        public string CreateCS( string className , string csv )
        {
            ...
            description = reader.GetRow( 0 );
            attribute = reader.GetRow( 1 );
            type = reader.GetRow( 2 );
        ...
        void AddBody()
        {
            ...
            {
                type[ i ] = CheckStringType( type[ i ] );
                CSString.Add( "/// <summary>" );
                CSString.Add( string.Format( "/// {0}" , description[ i ] ) );
                CSString.Add( "/// </summary>" );
                string subtype = type[ i ];
                if( subtype.Contains( "En_" ) )
                {
                    subtype = subtype.Substring( 3 );
                }
                CSString.Add( string.Format( "public {0} {1} {{ get; set; }}" , DicConvert( subtype ) , attribute[ i ] ) );
            }
            //添加方法
        ...
        void AddMethod( EnMethodType mtype )
        {
            ...
            if( mtype == EnMethodType.List )
                CSString.Add( "public static List<" + ClassName + "> ReturnList(string csv)" );
            else
                CSString.Add( string.Format( "public static Dictionary<{0}, {1}> ReturnDictionary(string csv)" , CheckEn( type[ 0 ] ) , ClassName ) );
            CSString.Add( "{" );
            if( mtype == EnMethodType.List )
                CSString.Add( string.Format( "List<{0}> vec = new List<{0}>();" , ClassName ) );
            else
                CSString.Add( string.Format( "Dictionary<{0}, {1}> vec = new Dictionary<{0}, {1}>();" , CheckEn( type[ 0 ] ) , ClassName ) );
            CSString.Add( "CSVReader reader = new CSVReader(csv);" );
            CSString.Add( "for (int i = 3; i < reader.Row; i++)" );
            CSString.Add( "{" );
            CSString.Add( string.Format( "{0} item = new {0}();" , ClassName ) );
            //中心
            for( int i = 0; i < type.Length; i++ )
            {
                if( type[ i ].Contains( "En_" ) )
                {
                    string subtype = type[ i ].Substring( 3 );
                    CSString.Add( string.Format( "item.{0} = {1};" , attribute[ i ] , ParseBaseType( subtype , string.Format( "reader.GetData({0}, i)" , i ) ) ) );
                }
                else if( type[ i ].Contains( "List<" ) )
                {
                    int stb = type[ i ].IndexOf( '<' );
                    int ste = type[ i ].Count() - 1;
                    string subtype = type[ i ].Substring( stb + 1 , ste - stb - 1 );
                    ...
                    CSString.Add( string.Format( "item.{0}= new List<{1}>();" , attribute[ i ] , subtype ) );
                    ...
                    if( subtype.Contains( "List<" ) )
                    {
                        ...
                        string usubstrName = string.Format( "{0}_Array2" , attribute[ i ] );//一级数组
                        string usublstname = string.Format( "{0}_List" , attribute[ i ] );//一级list名
                        ...
                        CSString.Add( string.Format( "item.{0}= new List<{1}>();" , attribute[ i ] , usubtype ) );
                        ...
                        CSString.Add( "}" );
                        CSString.Add( string.Format( "item.{0}.Add({1});" , attribute[ i ] , usublstname ) );
                    }
                    else
                    {
                        CSString.Add( string.Format( "item.{0}.Add({1});" , attribute[ i ] , ParseBaseType( subtype , string.Format( "{0}[j]" , substrName ) ) ) );

                    }
                    CSString.Add( "}" );

                }
                else if( type[ i ].Contains( "Dictionary<" ) )
                {
                    int stb = type[ i ].IndexOf( '<' );
                    ...
                    CSString.Add( string.Format( "item.{0}.Add({1}, {2});" , attribute[ i ] , ParseBaseType( subtype[ 0 ] , "subArray[0]" ) , ParseBaseType( subtype[ 1 ] , "subArray[1]" ) ) );
                    CSString.Add( "}" );
                }
                else
                    CSString.Add( string.Format( "item.{0} = {1};" , attribute[ i ] , ParseBaseType( type[ i ] , string.Format( "reader.GetData({0}, i)" , i ) ) ) );
            }
            //收尾
            if( mtype == EnMethodType.List )
                CSString.Add( "vec.Add(item);" );
            else
                CSString.Add( string.Format( "vec.Add(item.{0}, item);" , attribute[ 0 ] ) );
            CSString.Add( "}" );
