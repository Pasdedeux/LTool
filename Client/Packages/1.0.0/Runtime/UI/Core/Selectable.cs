...
namespace UnityEngine.UI
{
        ...
        ISelectHandler, IDeselectHandler
    {
        ...
        /// 缩放动画
        /// </summary>
        private TweenRunner<FloatTween> m_ScaleTweenRunner;
        /// <summary>
        ...
        /// </code>
        /// </example>
        private Graphic[] m_TargetAllGraphic;
        ...
        {
            m_ScaleTweenRunner = new TweenRunner<FloatTween>();
        ...
        protected override void Awake()
        {
            ...
            m_TargetAllGraphic = m_TargetTrans.GetComponentsInChildren<Graphic>();
        }
        ...
        protected virtual void DoStateTransition(SelectionState state, bool instant)
        {
            ...
            switch (m_Transition)
            {
                case Transition.ColorTint:
                    ...
                    break;
                case Transition.Scale:
                    StartScaleTween( state==SelectionState.Pressed,instant);
                    break;
                case Transition.SpriteSwap:
        ...
        {
            if (m_TargetAllGraphic == null)
                return;
            if(m_TargetAllGraphic.Length <= 0)
            {
                return;
            }

           foreach(Graphic graphic in m_TargetAllGraphic)
            {
                graphic.CrossFadeColor(targetColor, instant ? 0f : m_Colors.fadeDuration, true, true);
            }
        }

        void StartScaleTween( bool isPress, bool instant)
        {
            if(!m_TargetTrans)
            {
                return;
            ...
            if(transform.localScale==Vector3.one*end)
            {
                return;
            }
            FloatTween tween = new FloatTween { duration = 0.05f, startValue =start, targetValue = end };
            tween.AddOnChangedCallback(SetScale);
            tween.ignoreTimeScale = true;
            m_ScaleTweenRunner.StartTween(tween);
        }
        private void SetScale(float aScale)
        {
            m_TargetTrans.localScale = Vector3.one * aScale;
        }
