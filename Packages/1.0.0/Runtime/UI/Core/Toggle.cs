...
namespace UnityEngine.UI
{
    ...
    public class Toggle : Selectable, IPointerClickHandler, ISubmitHandler, ICanvasElement
    {
        ...
        [Header("选中音量")]
        [Range(0, 100)]
        public int m_BtnOKVolume = 100;
        [Header("不选音量")]
        [Range(0, 100)]
        public int m_BtnNOVolume = 100;
        [Header("选中音频")]
        public AudioClip m_BtnOKAudio;
        [Header("不选音频")]
        public AudioClip m_BtnNoAudio;
        [Serializable]
        /// <summary>
        ...
        {
            base.Awake();
            if (m_BtnOKVolume == 100)
            {
                m_BtnOKVolume = UGUIDefaultConfig.DefaultVolume;
            }
            if (m_BtnNOVolume == 100)
            {
                m_BtnNOVolume = UGUIDefaultConfig.DefaultVolume;
            }
            if (m_BtnOKAudio == null)
            {
                m_BtnOKAudio = UGUIDefaultConfig.DefaultAudioClip;
            }
            if (m_BtnNoAudio == null)
            {
                m_BtnNoAudio = UGUIDefaultConfig.DefaultAudioClip;
            }
            if (m_BtnOKAudio != null && m_BtnNoAudio != null)
            {
        ...
        /// 播放按钮音效
        /// </summary>
        private void PlayAudio(bool isOk)
        {
            if(isOk)
            {
                UGUIDefaultConfig.PlayAudio.Invoke(m_BtnOKAudio,m_BtnOKVolume);
            }
            else
            {
                UGUIDefaultConfig.PlayAudio.Invoke(m_BtnNoAudio,m_BtnNOVolume);
            }
        }
