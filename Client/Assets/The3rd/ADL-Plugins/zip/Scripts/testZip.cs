...
public class testZip : MonoBehaviour
{
    ...
    // We use here a temp lzip.inMemory struct to keep the pointer and size of the buffer
     IEnumerator NativeFileBufferDownload() {
        ...
        StartCoroutine(lzip.downloadZipFileNative("http://telias.free.fr/temp/testZip.zip", r => downloadDone2 = r, result => inMemZip = result));
        ...
        // Here we are calling the coroutine for a pointer. We also get the downloaded file size.
         StartCoroutine(lzip.downloadZipFileNative("http://telias.free.fr/temp/testZip.zip", r => downloadDone2 = r, null, pointerResult => nativePointer = pointerResult, size => zipSize = size));
