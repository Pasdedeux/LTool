...
public class AdapterMopub : BaseAdAdapter
{
    ...
    {
#if MOPUB
        Log.Info("IsIntersititialReady " + IsIntersititialReady());
        ...
        if (IsIntersititialReady())
        {
            ...
            if ( DataModel.Instance.CurrentLevel <= 20 )
            {
                ...
                Log.Info( "===>MoPub.ShowInterstitialAd _interstitialAdUnits[1]" );
            }
            else
            {
                MoPub.ShowInterstitialAd( _interstitialAdUnits[ 0 ] );
                Log.Info( "===>MoPub.ShowInterstitialAd _interstitialAdUnits[0]" );
            }
#elif UNITY_IOS
            if (DataModel.Instance.CurrentLevel <= 20)
            {
    ...
    {
        Log.Info("mopub调试——interstitial读取成功");
    ...
    {
        if (error != null) Log.Info("OnInterstitialFailedEvent " + error);
        if (InterstitialLoadEventHandler != null) InterstitialLoadEventHandler(false);
        Log.Info("mopub调试——interstitial读取失败, id:" + adUnitId);
    ...
    {
        //MoPub.RequestInterstitialAd(_interstitialAdUnits[0]);
        Log.Info("mopub调试——interstitial关闭");
    }

    //插屏激活回调
    private void OnInterstitialShownEvent(string adUnitId)
    {
        Log.Info("mopub调试——interstitial插屏激活回调,id:" + adUnitId);
    ...
    {
        Log.Info("mopub调试——interstitial单击的时候回调,id:" + adUnitId);
        if (InterstitialShowEventHandler != null) InterstitialShowEventHandler(true);
    }
    ...
    {
        //MoPub.RequestRewardedVideo(_rewardedVideoAdUnits[0]);
        Log.Info("激励视频广告加载失败,id:" + adUnitId);
        Log.Info("激励视频广告加载失败,error:" + error);
        RewardShowEventHandler?.Invoke(false);
    }
    ...
    {
        Log.Info("激励视频广告播放失败,id:" + adUnitId);
        Log.Info("激励视频广告播放失败,error:" + error);
    }
    ...
    {
        Log.Info("激励视频跳转其他应用,id:" + adUnitId);
    }
#endif
