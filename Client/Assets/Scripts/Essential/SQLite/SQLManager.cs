    ...
    public class SQLManager : SingletonMono<SQLManager>
    {
        ...
        public void Initialize(bool forceReinitialization)
        {
            if (_db == null || forceReinitialization)
            {
                ...
                string databaseFileName = FrameworkConfig.Instance.UsePersistantPath ? AssetPathManager.Instance.GetPersistentDataPath("csv/csvconfigs.bytes", false) : AssetPathManager.Instance.GetStreamAssetDataPath("csv/csvconfigs.bytes", false);
        ...
        ///// methods to set and get properties of objects.
        ///// </returns>
        //public TableMapping GetMapping(string typeName)
        //{
        //    Initialize(false);

        //    return _db.GetMapping(typeName);
        //}
        ...
        /// ILRruntiem not support "out/ref"
        public T QueryFirstRecord<T>(out bool recordExists, string query, params object[] args) where T : /*class, */new()
        {
            Initialize(false);
