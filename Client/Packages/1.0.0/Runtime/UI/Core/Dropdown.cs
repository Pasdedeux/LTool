...
namespace UnityEngine.UI
{
    ...
    public class Dropdown : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler
    {
        [Header("下拉音量")]
        [Range(0, 100)]
        public int m_DropDownVolume = 100;
        [Header("选项音量")]
        [Range(0, 100)]
        public int m_DropItemVolume = 100;
        [Header("下拉音频")]
        public AudioClip m_DropDownAudio;
        [Header("选项音频")]
        public AudioClip m_DropItemAudio;
        ...
        /// 播放下拉音效
        /// </summary>
        private void PlayDownAudio()
        {
            if(m_DropDownAudio)
            {
                UGUIDefaultConfig.PlayAudio.Invoke(m_DropDownAudio, m_DropDownVolume);
            }
        }
        /// <summary>
        /// 播放选择音效
        /// </summary>
        private void PlayItemAudio(int itemIndex)
        {
            UGUIDefaultConfig.PlayAudio.Invoke(m_DropDownAudio, m_DropItemVolume);
        }

        protected override void Awake()
        {
            if (m_DropDownVolume == 100)
            {
                m_DropDownVolume = UGUIDefaultConfig.DefaultVolume;
            }
            if (m_DropItemVolume == 100)
            {
                m_DropItemVolume = UGUIDefaultConfig.DefaultVolume;
            }
            if (m_DropDownAudio == null)
            {
                m_DropDownAudio = UGUIDefaultConfig.DefaultAudioClip;
            }
            if (m_DropItemAudio == null)
            {
                m_DropDownAudio = UGUIDefaultConfig.DefaultAudioClip;
            }
            if (m_DropDownAudio != null)
            {
                m_OnValueChanged.AddListener(PlayItemAudio);
            }
#if UNITY_EDITOR
